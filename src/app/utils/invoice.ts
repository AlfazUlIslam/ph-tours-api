...
const generatePdf = async (invoiceData: IInvoiceData): Promise<Buffer<ArrayBufferLike>> => {
    try {
        return new Promise((resolve, reject) => {
            ...
            doc.text(`Custommer : ${invoiceData.userName}`);
            doc.moveDown();
        ...
        console.log(error);
        throw new AppError(401, `PDF creation error: ${error}`)
    }
};
